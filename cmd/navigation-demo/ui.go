...
func newDemoUI(env demoEnvironment) (*demoUI, error) {
	...
	if env.DataDir == "" {
		return nil, errors.New("demo: data directory is required")
	}
	env, cancel := env.prepare()
	u := &demoUI{
		env: env, cancel: cancel,
		table:       table,
		router:      r,
		statePath:   filepath.Join(env.DataDir, "navigation-state.json"),
	...
	u.gridDemo = newGridDemo(env)
	u.persistence = newStateWriter(u.statePath, env.Invalidate)
...
func (u *demoUI) installGuard() {
	u.router.SetGuard(func(transition router.Transition) bool {
		dirty := u.dirty
		if form := u.formDemos[transition.From.Params["id"].String()]; form != nil {
			// A field can commit after this frame's status snapshot. Back must
			// consult the controller, not a cached flag from the prior layout.
			dirty = form.form.IsDirty()
		}
		if dirty && transition.From.Name == "incident.form" && transition.Operation != router.PushModalOperation {
			u.dirty = true
			u.confirming = true
...
func (u *demoUI) Layout(gtx layout.Context) layout.Dimensions {
	...
	u.gridDemo.poll(u)
	select {
	case result := <-u.persistence.results:
		if result.version == u.lastVersion && result.err != nil {
			u.fail(result.err)
		}
	default:
	}
	...
	case result := <-u.externalNav:
		u.externalBusy = false
		if result.err == nil {
			result.err = u.router.Push(result.route)
		}
		if result.err != nil {
			u.fail(result.err)
		} else {
	...
	if u.router.Version() != u.lastVersion {
		...
		if data, err := u.router.Serialize(); err == nil {
			u.persistence.submit(u.lastVersion, data)
		}
...
func (u *demoUI) listScreen(gtx layout.Context) layout.Dimensions {
	return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
		...
		layout.Rigid(func(gtx layout.Context) layout.Dimensions {
			if u.notification.Clicked(gtx) && !u.externalBusy {
				...
				go func() {
					defer func() { u.work.end(); u.env.Invalidate() }()
					if err := u.env.Sleep(u.env.Context, 700*time.Millisecond); err != nil {
						return
					}
					route, err := u.table.ResolveURL("gio-kit://notification/incident/INC0042?openedFrom=notification")
					select {
					case u.externalNav <- externalResult{message: "Background notification pushed INC0042.", route: route, err: err}:
					case <-u.env.Context.Done():
						return
					}
					u.env.Invalidate()
				}()
