...
func compileAndroid(tmpDir string, tools *androidTools, bi *buildInfo) (err error) {
	...
	if len(javaFiles) > 0 {
		...
		pkgDir, err := runCmd(exec.Command("go", "list", "-tags", bi.tags, "-f", "{{.Dir}}", bi.pkgPath))
		if err != nil {
			return err
		}
		projectJava, err := filepath.Glob(filepath.Join(pkgDir, "android", "*.java"))
		if err != nil {
			return err
		}
		javaFiles = append(javaFiles, projectJava...)
		...
		javac := exec.Command(
			...
			"-target", "1.8",
			"-source", "1.8",
			"-sourcepath", appDir+string(os.PathListSeparator)+filepath.Join(pkgDir, "android"),
...
func findNDK(androidHome string) (string, error) {
	if pinned := os.Getenv("ANDROID_NDK_ROOT"); pinned != "" {
...
func archNDK() string {
	...
	case "arm64":
		if runtime.GOOS == "darwin" {
			...
			arch = "x86_64"
		} else if runtime.GOOS == "android" || runtime.GOOS == "linux" {
			// Termux's patched arm64 NDK preserves the upstream host directory name.
			return "linux-x86_64"
		} else {
...
func latestPlatform(sdk string) (string, error) {
	if pinned := os.Getenv("GIO_ANDROID_PLATFORM"); pinned != "" {
		path := filepath.Join(sdk, "platforms", "android-"+pinned)
		if _, err := os.Stat(filepath.Join(path, "android.jar")); err != nil {
			return "", fmt.Errorf("pinned platform %s: %w", pinned, err)
		}
		return path, nil
	}
...
func latestCompiler(tcRoot, a string, minsdk int) (string, error) {
	...
	// Go's Android-hosted cgo resolves absolute CC paths relative to its temp dir.
	// The build script supplies the NDK library search path separately.
	if runtime.GOOS == "android" && a == "arm64" {
		return "aarch64-linux-android-clang", nil
	}
	return filepath.Base(bestCompiler), nil
}

func latestTools(sdk string) (string, error) {
	if pinned := os.Getenv("GIO_ANDROID_BUILD_TOOLS"); pinned != "" {
		path := filepath.Join(sdk, "build-tools", pinned)
		if _, err := os.Stat(path); err != nil {
			return "", fmt.Errorf("pinned build-tools %s: %w", pinned, err)
		}
		return path, nil
	}
