...
out=$project_dir/build/gio-kit-form-phase3-release.apk
dest=/storage/emulated/0/Download/gio-kit-form-phase3-release.apk
...
"$gogio_bin" -target android -arch arm64 -appid app.giokit.cruddemo \
	-name "Gio Kit CRUD Lab" -version 0.3.1.10 -minsdk 24 -targetsdk 35 \
	-schemes gio-kit -signkey "$HOME/.android/debug.keystore" -signpass android \
	-o "$out" ./cmd/navigation-demo

native_check=$project_dir/build/libgio-form-check.so
